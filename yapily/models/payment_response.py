...
class PaymentResponse(BaseModel):
    ...
    PaymentResponse
    """
    id: Optional[StrictStr] = Field(None, description="Unique identifier of the payment.")
    ...
    payment_idempotency_id: Optional[StrictStr] = Field(None, alias="paymentIdempotencyId", description="__Mandatory__. A unique identifier that you must provide to identify the payment. This can be any alpha-numeric string but is limited to a maximum of 35 characters.")
    ...
    amount: Optional[Union[StrictFloat, StrictInt]] = Field(None, description="Monetary amount.")
    currency: Optional[StrictStr] = Field(None, description="Currency the payment amount is denoted in. Specified as a 3-letter ISO 4217 code.")
    amount_details: Optional[Amount] = Field(None, alias="amountDetails")
    created_at: Optional[datetime] = Field(None, alias="createdAt", description="Date and time of when the payment request was created.")
    first_payment_amount: Optional[Amount] = Field(None, alias="firstPaymentAmount")
    first_payment_date_time: Optional[datetime] = Field(None, alias="firstPaymentDateTime", description="Date and time of when the first payment request is to be made.")
    next_payment_amount: Optional[Amount] = Field(None, alias="nextPaymentAmount")
    next_payment_date_time: Optional[datetime] = Field(None, alias="nextPaymentDateTime", description="__Conditional__. Defines when the recurring payment is to be made.")
    final_payment_amount: Optional[Amount] = Field(None, alias="finalPaymentAmount")
    final_payment_date_time: Optional[datetime] = Field(None, alias="finalPaymentDateTime", description="Date and time of when the final payment is to be made.")
    number_of_payments: Optional[StrictInt] = Field(None, alias="numberOfPayments", description="Number of recurring payment requests to be made as part of the instructed payment schedule.")
    previous_payment_amount: Optional[Amount] = Field(None, alias="previousPaymentAmount")
    previous_payment_date_time: Optional[datetime] = Field(None, alias="previousPaymentDateTime", description="Date and time of when the previous payment request was posted.")
    charge_details: Optional[conlist(PaymentChargeDetails)] = Field(None, alias="chargeDetails")
    scheduled_payment_type: Optional[StrictStr] = Field(None, alias="scheduledPaymentType", description="Details the execution type and the payment date between the payer and the payee.")
    scheduled_payment_date_time: Optional[datetime] = Field(None, alias="scheduledPaymentDateTime", description="Date and time of when the scheduled payment request will be made.")
    frequency: Optional[FrequencyResponse] = None
    currency_of_transfer: Optional[StrictStr] = Field(None, alias="currencyOfTransfer", description="__Mandatory__. The currency to be transferred to the payee. This may differ from the currency the payment is denoted in and the currency of the payer's account. Specified as a 3-letter code (ISO 4217).")
    purpose: Optional[StrictStr] = Field(None, description="Specifies the external purpose code for the `Institution` - IS0 20022.")
