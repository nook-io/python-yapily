...
class HostedAccountRequest(BaseModel):
    ...
    transaction_from: Annotated[datetime | None, Field(alias='transactionFrom', description='__Optional__. Specifies the earliest date of the transaction records to be returned.<br><br> You must supply this field to retrieve transactions older than 90 days for banks accessed via the the [CBI Globe Gateway](https://docs.yapily.com/pages/data/financial-data-resources/data-restrictions/#cbi-globe-gateway).')] = None
    transaction_to: Annotated[datetime | None, Field(alias='transactionTo', description='__Optional__. Specifies the latest date of the transaction records to be returned.')] = None
    expires_at: Annotated[datetime | None, Field(alias='expiresAt', description="__Optional__. Used to set a hard date for when the user's associated `Consent` will expire.<br><br>**Note**: If this supported by the bank, specifying this is property is opting out of having a long-lived consent that can be perpetually re-authorised by the user. This will add an `expiresAt` field on the `Consent` object which will render it unusable after this date.<br><br>**Note**: This is not supported by every `Institution`. In such case, the request will not fail but the property will be ignored and the created `Consent` will not have an expiry date.")] = None
