...
class Transaction(BaseModel):
    ...
    """
    id: Optional[StrictStr] = Field(None, description="Unique identifier of the transaction.")
    var_date: Optional[datetime] = Field(None, alias="date")
    booking_date_time: Optional[datetime] = Field(None, alias="bookingDateTime", description="Date and time of when a transaction was booked.")
    value_date_time: Optional[datetime] = Field(None, alias="valueDateTime", description="Date and time when the funds either cease to be available (for debit transactions) or become available (for credit transactions) to the account owner.")
    status: Optional[TransactionStatusEnum] = None
    amount: Optional[Union[StrictFloat, StrictInt]] = Field(None, description="The transaction amount.")
    currency: Optional[StrictStr] = Field(None, description="Currency the transaction amount is denoted in. Specified as a 3-letter ISO 4217 code.")
    ...
    transaction_information: Optional[conlist(StrictStr)] = Field(None, alias="transactionInformation", description="Further details on the transaction. This is narrative data, caught as unstructured text.")
    ...
    payee_details: Optional[TransactionPayeeDetails] = Field(None, alias="payeeDetails")
    payer_details: Optional[TransactionPayerDetails] = Field(None, alias="payerDetails")
    merchant: Optional[Merchant] = None
    enrichment: Optional[Enrichment] = None
    supplementary_data: Optional[Dict[str, Any]] = Field(None, alias="supplementaryData", description="Additional information that cannot be captured in a structured field or block.")
    transaction_mutability: Optional[StrictStr] = Field(None, alias="transactionMutability", description="__Optional__. Specifies the Mutability of the Transaction record.<ul><li>A transaction with a `Status` of `Pending` is mutable.</li><li>A transaction with a `Status` of `Booked` where the `TransactionMutability` flag is not specified is not guaranteed to be immutable (although in most instances it will be).</li><li>A transaction with a `Status` of `Booked` with the `TransactionMutability` flag set to `Immutable` is immutable.</li><li>A transaction with a `Status` of `Booked` with the `TransactionMutability` flag set to `Mutable` is mutable.</li></ul>")
    __properties = ["id", "date", "bookingDateTime", "valueDateTime", "status", "amount", "currency", "transactionAmount", "grossAmount", "currencyExchange", "chargeDetails", "reference", "statementReferences", "description", "transactionInformation", "addressDetails", "isoBankTransactionCode", "proprietaryBankTransactionCode", "balance", "payeeDetails", "payerDetails", "merchant", "enrichment", "supplementaryData", "transactionMutability"]
    ...
    def from_dict(cls, obj: dict) -> Transaction:
        ...
        _obj = Transaction.parse_obj({
            ...
            "payee_details": TransactionPayeeDetails.from_dict(obj.get("payeeDetails")) if obj.get("payeeDetails") is not None else None,
            "payer_details": TransactionPayerDetails.from_dict(obj.get("payerDetails")) if obj.get("payerDetails") is not None else None,
