...
class AccountAuthorisationResponse(BaseModel):
    ...
    """
    id: Optional[StrictStr] = Field(None, description="Unique identifier for the account authorisation request.<br><br>The `consentID` used to [retrieve a consent](/api/reference/#operation/getConsentById).")
    ...
    last_confirmed_at: Optional[datetime] = Field(None, alias="lastConfirmedAt", description="The time the PSU last confirmed access to their account information, either through full authentication with the institution, or through reconfirmation with the TPP.")
    reconfirm_by: Optional[datetime] = Field(None, alias="reconfirmBy", description="The date and time the consent must be reconfirmed by to ensure continued access to the account information.")
    ...
    __properties = ["id", "userUuid", "applicationUserId", "referenceId", "institutionId", "status", "createdAt", "transactionFrom", "transactionTo", "expiresAt", "timeToExpireInMillis", "timeToExpire", "featureScope", "consentToken", "state", "authorizedAt", "lastConfirmedAt", "reconfirmBy", "institutionConsentId", "authorisationUrl", "qrCodeUrl"]
    ...
    def from_dict(cls, obj: dict) -> AccountAuthorisationResponse:
        ...
        _obj = AccountAuthorisationResponse.parse_obj({
            "id": obj.get("id"),
