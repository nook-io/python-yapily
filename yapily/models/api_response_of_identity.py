...
class ApiResponseOfIdentity(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> "ApiResponseOfIdentity":
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> "ApiResponseOfIdentity":
        """Create an instance of ApiResponseOfIdentity from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ApiResponseOfIdentity.model_validate(obj)

        return ApiResponseOfIdentity.model_validate(
            {
                "meta": ResponseMeta.from_dict(obj.get("meta")) if obj.get("meta") is not None else None,
                "data": Identity.from_dict(obj.get("data")) if obj.get("data") is not None else None,
                "links": obj.get("links"),
