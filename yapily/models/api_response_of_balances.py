...
class ApiResponseOfBalances(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> "ApiResponseOfBalances":
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> "ApiResponseOfBalances":
        """Create an instance of ApiResponseOfBalances from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ApiResponseOfBalances.model_validate(obj)

        return ApiResponseOfBalances.model_validate(
            {
                "meta": ResponseMeta.from_dict(obj.get("meta")) if obj.get("meta") is not None else None,
                "data": Balances.from_dict(obj.get("data")) if obj.get("data") is not None else None,
                "links": obj.get("links"),
