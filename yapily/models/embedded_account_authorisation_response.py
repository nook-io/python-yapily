...
class EmbeddedAccountAuthorisationResponse(BaseModel):
    ...
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="`User` for which the authorisation request was created.")
    user_uuid: Optional[StrictStr] = Field(default=None, description="__Conditional__. User-friendly identifier of the `User` that provides authorisation. If a `User` with the specified `applicationUserId` exists, it will be used otherwise, a new `User` with the specified `applicationUserId` will be created and used. Either the `userUuid` or `applicationUserId` must be provided.", alias="userUuid")
    ...
    institution_id: Optional[StrictStr] = Field(default=None, description="__Mandatory__. The `Institution` the authorisation request is sent to.", alias="institutionId")
    status: Optional[AuthorisationStatus] = None
    created_at: Optional[datetime] = Field(default=None, description="Date and time of when the embedded authorisation was created by the application user.", alias="createdAt")
    ...
    expires_at: Optional[datetime] = Field(default=None, description="Date and time of when the embedded authorisation will expire by. Reauthorisation will be needed to retain access.", alias="expiresAt")
    ...
    feature_scope: Optional[List[FeatureEnum]] = Field(default=None, description="The set of features that the consent will provide access to.", alias="featureScope")
    consent_token: Optional[StrictStr] = Field(default=None, description="Represents the authorisation to gain access to the requested features. Required to access account information or make a payment request.", alias="consentToken")
    state: Optional[StrictStr] = Field(default=None, description="Corellation ID used when handshaking with a new insitution via OAuth2 registration.")
    authorized_at: Optional[datetime] = Field(default=None, description="Date and time of when the request was authorised by the Institution.", alias="authorizedAt")
    ...
    qr_code_url: Optional[StrictStr] = Field(default=None, description="The URL link for the QR code that may be scanned via a mobile device to make a authorisation redirect to the bank (authURL encoded).", alias="qrCodeUrl")
    sca_methods: Optional[List[ScaMethod]] = Field(default=None, description="List of `SCA methods` the `Institution` supports and that are available for selection.", alias="scaMethods")
    selected_sca_method: Optional[ScaMethod] = Field(default=None, alias="selectedScaMethod")
    __properties: ClassVar[List[str]] = ["id", "userUuid", "applicationUserId", "referenceId", "institutionId", "status", "createdAt", "transactionFrom", "transactionTo", "expiresAt", "timeToExpireInMillis", "timeToExpire", "featureScope", "consentToken", "state", "authorizedAt", "institutionConsentId", "authorisationUrl", "qrCodeUrl", "scaMethods", "selectedScaMethod"]
