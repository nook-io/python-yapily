...
class ApiResponseOfConsent(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> "ApiResponseOfConsent":
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> "ApiResponseOfConsent":
        """Create an instance of ApiResponseOfConsent from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ApiResponseOfConsent.model_validate(obj)

        return ApiResponseOfConsent.model_validate(
            {
                "meta": ResponseMeta.from_dict(obj.get("meta")) if obj.get("meta") is not None else None,
                "data": Consent.from_dict(obj.get("data")) if obj.get("data") is not None else None,
                "links": obj.get("links"),
