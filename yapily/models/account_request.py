...
class AccountRequest(BaseModel):
    ...
    """
    transaction_from: Optional[datetime] = Field(None, alias="transactionFrom", description="__Optional__. Used to specify the lower bound on when to pull transaction. This should be declared when accessing transaction older than 90 days for banks in the [CBI Globe](https://docs.yapily.com/pages/knowledge/open-banking/cbi_globe/).")
    transaction_to: Optional[datetime] = Field(None, alias="transactionTo", description="__Optional__. When performing a request using the consent, this is the latest date of transaction records that can be retrieved.")
    expires_at: Optional[datetime] = Field(None, alias="expiresAt", description="__Optional__. Used to set a hard date for when the user's associated `Consent` will expire.<br><br>**Note**: If this supported by the bank, specifying this is property is opting out of having a long-lived consent that can be perpetually re-authorised by the user. This will add an `expiresAt` field on the `Consent` object which will render it unusable after this date.<br><br>**Note**: This is not supported by every `Institution`. In such case, the request will not fail but the property will be ignored and the created `Consent` will not have an expiry date.")
    account_identifiers: Optional[AccountInfo] = Field(None, alias="accountIdentifiers")
    account_identifiers_for_transaction: Optional[conlist(AccountInfo)] = Field(None, alias="accountIdentifiersForTransaction", description="__Conditional__. Used to create a request for the transactions of the account specified. Once the user authorises the request, only the transactions can be obtained by executing [GET Account Transactions](./#get-account-transactions). <br><br>This can be specified in conjunction with `accountIdentifiersForBalance` to generate a `Consent` that can both access the accounts balance and transactions.")
    account_identifiers_for_balance: Optional[conlist(AccountInfo)] = Field(None, alias="accountIdentifiersForBalance", description="__Conditional__. Used to create a request for the balance of the account specified. Once the user authorises the request, only the balance can be obtained by executing [GET Account Balances](./#get-account-balances).<br><br> This can be specified in conjunction with `accountIdentifiersForTransaction` to generate a `Consent` that can both access the accounts balance and transactions.")
    ...
    __properties = ["transactionFrom", "transactionTo", "expiresAt", "accountIdentifiers", "accountIdentifiersForTransaction", "accountIdentifiersForBalance", "featureScope"]
    ...
    def from_dict(cls, obj: dict) -> AccountRequest:
        ...
        _obj = AccountRequest.parse_obj({
            ...
            "account_identifiers": AccountInfo.from_dict(obj.get("accountIdentifiers")) if obj.get("accountIdentifiers") is not None else None,
            "account_identifiers_for_transaction": [AccountInfo.from_dict(_item) for _item in obj.get("accountIdentifiersForTransaction")] if obj.get("accountIdentifiersForTransaction") is not None else None,
            "account_identifiers_for_balance": [AccountInfo.from_dict(_item) for _item in obj.get("accountIdentifiersForBalance")] if obj.get("accountIdentifiersForBalance") is not None else None,
