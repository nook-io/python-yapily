...
class VirtualAccountPaymentSource(BaseModel):
    ...
    account_id: Optional[StrictStr] = Field(default=None, description="Only present if type is ACCOUNT. Identifies the Virtual Account from which the payment was made", alias="accountId")
    account_identifications: Optional[List[AccountIdentification]] = Field(default=None, description="Only present if type is EXTERNAL. The account identifications that identify an external source", alias="accountIdentifications")
    __properties: ClassVar[List[str]] = ["type", "accountId", "accountIdentifications"]
