...
class VrpConfiguration(BaseModel):
    ...
    VrpConfiguration
    """
    maximum_individual_amount: Optional[Amount] = Field(None, alias="maximumIndividualAmount")
    maximum_cumulative_amount: Optional[Amount] = Field(None, alias="maximumCumulativeAmount")
    maximum_cumulative_number_of_payments: Optional[conint(strict=True, ge=0)] = Field(None, alias="maximumCumulativeNumberOfPayments", description="Maximum cumulative number of payments")
    periodic_limits: Optional[conlist(VrpPeriodicLimit, min_items=1)] = Field(None, alias="periodicLimits")
    recurring_payment_category: Optional[StrictStr] = Field(None, alias="recurringPaymentCategory", description="Payment Category with allowed values: ONGOING, SUBSCRIPTION")
    __properties = ["maximumIndividualAmount", "maximumCumulativeAmount", "maximumCumulativeNumberOfPayments", "periodicLimits", "recurringPaymentCategory"]
    ...
    def from_dict(cls, obj: dict) -> VrpConfiguration:
        ...
        _obj = VrpConfiguration.parse_obj({
            "maximum_individual_amount": Amount.from_dict(obj.get("maximumIndividualAmount")) if obj.get("maximumIndividualAmount") is not None else None,
            "maximum_cumulative_amount": Amount.from_dict(obj.get("maximumCumulativeAmount")) if obj.get("maximumCumulativeAmount") is not None else None,
            "maximum_cumulative_number_of_payments": obj.get("maximumCumulativeNumberOfPayments"),
            "periodic_limits": [VrpPeriodicLimit.from_dict(_item) for _item in obj.get("periodicLimits")] if obj.get("periodicLimits") is not None else None,
