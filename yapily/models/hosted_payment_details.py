...
class HostedPaymentDetails(BaseModel):
    ...
    """ # noqa: E501
    payment_request_id: Optional[StrictStr] = Field(default=None, description="The unique ID of the payment request.", alias="paymentRequestId")
    user_id: Optional[StrictStr] = Field(default=None, description="The Unique Identifier for the `User` assigned by Yapily.", alias="userId")
    application_user_id: Optional[StrictStr] = Field(default=None, description="Your reference to the `User`.", alias="applicationUserId")
    application_id: Optional[StrictStr] = Field(default=None, description="The Unique Identifier of the `Application` the user is associated with.", alias="applicationId")
    ...
    redirect_url: Optional[StrictStr] = Field(default=None, description="URL of your server to redirect the user after completion of the payment flow.", alias="redirectUrl")
    payment_request_details: Optional[HostedPaymentRequestDetailsLink] = Field(default=None, alias="paymentRequestDetails")
    created_at: Optional[datetime] = Field(default=None, description="The date and time at which the payment request was created.", alias="createdAt")
    ...
    status: Optional[StrictStr] = Field(default=None, description="Current status of the payment request. <br> Possible values: <br> ACTIVE <br> INACTIVE")
    payments: Optional[List[HostedPayment]] = Field(default=None, description="Payments that have been initiated as part of this request")
    __properties: ClassVar[List[str]] = ["paymentRequestId", "userId", "applicationUserId", "applicationId", "institutionIdentifiers", "userSettings", "redirectUrl", "paymentRequestDetails", "createdAt", "authorisationExpiresAt", "status", "payments"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "paymentRequestDetails": HostedPaymentRequestDetailsLink.from_dict(obj["paymentRequestDetails"]) if obj.get("paymentRequestDetails") is not None else None,
            "createdAt": obj.get("createdAt"),
            "authorisationExpiresAt": obj.get("authorisationExpiresAt"),
            "status": obj.get("status"),
            "payments": [HostedPayment.from_dict(_item) for _item in obj["payments"]] if obj.get("payments") is not None else None
        })
        return _obj
