...
class NonSweepingPeriodicLimits(BaseModel):
    ...
    frequency: Optional[StrictStr] = Field(None, description="__Mandatory__. Frequency for which the payment limits are enforced. Allowed values are [DAILY, WEEKLY, FORTNIGHTLY, MONTHLY, YEARLY]. This field cannot be enforced when `recurringPaymentCategory=ONGOING`.")
    alignment: Optional[StrictStr] = Field(None, description="__Mandatory__. Period alignment for which the payment limits are enforced. Allowed values are [CONSENT, CALENDAR]. If CONSENT, then period starts on consent creation date. If CALENDAR, then period lines up with the frequency e.g. WEEKLY period will begin at start of the week in question. This field cannot be enforced when `recurringPaymentCategory=ONGOING`.")
    max_number_of_payments: Optional[StrictInt] = Field(None, alias="maxNumberOfPayments", description="__Optional__. Max number of payments that can be submitted under this period.")
