...
class PaymentEmbeddedAuthorisationRequest(BaseModel):
    ...
    sca_code: Optional[StrictStr] = Field(None, alias="scaCode", description="__Conditional__. Used to update the authorisation with the sca code received by the user from the `Institution` using the embedded payment authorisation flow.<br><br>This is the penultimate step required in the embedded payment authorisation flow to authorise the `Consent`. After sending the sca code, to obtain an authorised consent, the last step is to poll [Get Consent](https://docs.yapily.com/api/reference/#operation/getConsentById) until the `Institution` authorises the request and the `Consent` `status` transitions to `AUTHORIZED`.")
    __properties = ["userUuid", "applicationUserId", "institutionId", "callback", "redirect", "oneTimeToken", "paymentRequest", "userCredentials", "selectedScaMethod", "scaCode"]
    ...
    def from_dict(cls, obj: dict) -> PaymentEmbeddedAuthorisationRequest:
        ...
        _obj = PaymentEmbeddedAuthorisationRequest.parse_obj({
