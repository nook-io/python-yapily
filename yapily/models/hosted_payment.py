...
class HostedPayment(BaseModel):
    ...
    HostedPayment
    """ # noqa: E501
    payment_id: Optional[StrictStr] = Field(default=None, description="The Unique Identifier of the payment.", alias="paymentId")
    hosted_payment_id: Optional[StrictStr] = Field(default=None, description="The Unique Identifier of the payment created using Yapily hosted application.", alias="hostedPaymentId")
    consent_id: Optional[StrictStr] = Field(default=None, description="The Unique Identifier of the consent.", alias="consentId")
    institution_identifiers: Optional[InstitutionIdentifiersResponse] = Field(default=None, alias="institutionIdentifiers")
    phases: Optional[List[HostedPaymentPhase]] = Field(default=None, description="The phase reached by the payment and its timestamp.")
    payment_status: Optional[StrictStr] = Field(default=None, description="Payment status based on latest HostedAuthPaymentPhase in phases. Value can be <ul> <li>PENDING  -  Payment pending processing</li> <li>COMPLETED  -  Payment processing completed</li> <li>FAILED  -  Payment process failed</li></ul>", alias="paymentStatus")
    status_details: Optional[List[HostedPaymentStatusDetails]] = Field(default=None, description="Details of the payment status.", alias="statusDetails")
    institution_payment_id: Optional[StrictStr] = Field(default=None, description="The Unique Identifier of the payment created with the `Institution`.", alias="institutionPaymentId")
    payment_lifecycle_id: Optional[StrictStr] = Field(default=None, description="The Unique Identifier provided by TPP in the Payment request to identify the payment.", alias="paymentLifecycleId")
    payment_idempotency_id: Optional[StrictStr] = Field(default=None, description="A unique identifier that you must provide to identify the payment. This can be any alpha-numeric string but is limited to a maximum of 35 characters.", alias="paymentIdempotencyId")
    ...
    __properties: ClassVar[List[str]] = ["paymentId", "hostedPaymentId", "consentId", "institutionIdentifiers", "phases", "paymentStatus", "statusDetails", "institutionPaymentId", "paymentLifecycleId", "paymentIdempotencyId", "reference", "contextType", "type", "payee", "payer", "amount"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "phases": [HostedPaymentPhase.from_dict(_item) for _item in obj["phases"]] if obj.get("phases") is not None else None,
            "paymentStatus": obj.get("paymentStatus"),
            "statusDetails": [HostedPaymentStatusDetails.from_dict(_item) for _item in obj["statusDetails"]] if obj.get("statusDetails") is not None else None,
            ...
            "amount": AmountDetailsResponse.from_dict(obj["amount"]) if obj.get("amount") is not None else None
        })
        return _obj
