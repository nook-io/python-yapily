...
class EmbeddedAccountAuthorisationRequest(BaseModel):
    ...
    callback: Optional[StrictStr] = Field(None, description="__Optional__. The server to redirect the user to after the user complete the authorisation at the `Institution`. <br><br>See [Using a callback (Optional)](https://docs.yapily.com/) for more information.")
    ...
    user_credentials: Optional[UserCredentials] = Field(None, alias="userCredentials")
    selected_sca_method: Optional[ScaMethod] = Field(None, alias="selectedScaMethod")
    sca_code: Optional[StrictStr] = Field(None, alias="scaCode", description="__Conditional__. Used to update the authorisation with the sca code received by the user from the `Institution` using the embedded account authorisation flow.<br><br>This is the penultimate step required in the embedded account authorisation flow to authorise the `Consent`. After sending the sca code, to obtain an authorised consent, the last step is to poll [Get Consent](https://docs.yapily.com/api/reference/#operation/getConsentById) until the `Institution` authorises the request and the `Consent` `status` transitions to `AUTHORIZED`.")
    account_request: Optional[AccountRequest] = Field(None, alias="accountRequest")
    __properties = ["userUuid", "applicationUserId", "forwardParameters", "institutionId", "callback", "redirect", "oneTimeToken", "userCredentials", "selectedScaMethod", "scaCode", "accountRequest"]
    ...
    def from_dict(cls, obj: dict) -> EmbeddedAccountAuthorisationRequest:
        ...
        _obj = EmbeddedAccountAuthorisationRequest.parse_obj({
            ...
            "user_credentials": UserCredentials.from_dict(obj.get("userCredentials")) if obj.get("userCredentials") is not None else None,
