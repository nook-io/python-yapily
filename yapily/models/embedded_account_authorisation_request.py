...
class EmbeddedAccountAuthorisationRequest(BaseModel):
    ...
    callback: Optional[StrictStr] = Field(default=None, description="__Optional__. The server to redirect the user to after the user complete the authorisation at the `Institution`. <br><br>See [Using a callback (Optional)](https://docs.yapily.com/) for more information.")
    ...
    sca_code: Optional[StrictStr] = Field(default=None, description="__Conditional__. Used to update the authorisation with the sca code received by the user from the `Institution` using the embedded account authorisation flow.<br><br>This is the penultimate step required in the embedded account authorisation flow to authorise the `Consent`. After sending the sca code, to obtain an authorised consent, the last step is to poll [Get Consent](https://docs.yapily.com/api/reference/#operation/getConsentById) until the `Institution` authorises the request and the `Consent` `status` transitions to `AUTHORIZED`.", alias="scaCode")
    account_request: Optional[AccountRequest] = Field(default=None, alias="accountRequest")
    __properties: ClassVar[List[str]] = ["userUuid", "applicationUserId", "forwardParameters", "institutionId", "callback", "redirect", "oneTimeToken", "userCredentials", "selectedScaMethod", "scaCode", "accountRequest"]
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "accountRequest": AccountRequest.from_dict(obj.get("accountRequest")) if obj.get("accountRequest") is not None else None
        })
        return _obj
